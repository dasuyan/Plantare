...
constructor(
  ...
  fun onAddClick(openScreen: (String) -> Unit) = openScreen("$EDIT_PLANT_SCREEN?$EDIT_PLANT_SCREEN_MODE={${EditPlantScreenMode.ADD.name}}?$PLANT_ID={null}")

  fun onSettingsClick(openScreen: (String) -> Unit) = openScreen(SETTINGS_SCREEN)

  fun onPlantClick(openScreen: (String) -> Unit, plant: Plant) {
    openScreen("$EDIT_PLANT_SCREEN?$EDIT_PLANT_SCREEN_MODE={${EditPlantScreenMode.DETAILS.name}}?$PLANT_ID={${plant.id}}")
  }
  ...
  fun onWaterClick(context: Context, plant: Plant) {
    ...
    launchCatching {
      ...
      if (
        plant.lastWateringDate.isNotBlank() && plant.wateringFrequencyDays.isNotBlank()
      ) {
